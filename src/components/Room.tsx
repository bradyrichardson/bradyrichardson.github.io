...
import { Fragment, JSX, forwardRef, useEffect, useRef, useState } from "react";
...
export default function Room({ ...props }): JSX.Element {
  ...
  const handleProjectDisplayBoardClick = (
    link: string,
    boardNumber: number
  ) => {
    if (
      projectDisplayBoardOneRef.current &&
      projectDisplayBoardTwoRef.current &&
      projectDisplayBoardThreeRef.current &&
      projectDisplayBoardFourRef.current
    ) {
      switch (boardNumber) {
        case 1:
          ...
          if (
            projectDisplayBoardOneRef.current.position.y >
            displayBoardStartPosition.y + 0.05
          ) {
        ...
        case 2:
          if (
            projectDisplayBoardTwoRef.current.position.y >
            displayBoardStartPosition.y + 0.05
          ) {
            window.open(link, "_blank");
          }
        ...
        case 3:
          if (
            projectDisplayBoardThreeRef.current.position.y >
            displayBoardStartPosition.y + 0.05
          ) {
            window.open(link, "_blank");
          }
        ...
        case 4:
          if (
            projectDisplayBoardFourRef.current.position.y >
            displayBoardStartPosition.y + 0.05
          ) {
            window.open(link, "_blank");
          }
  ...
  useFrame((_, delta) => {
    ...
    if (drawerThreeRef.current && targetPositionThree.current) {
      drawerThreeRef.current.position.lerp(
        targetPositionThree.current,
        delta * 5
      );
    }
    if (drawerFourRef.current && targetPositionFour.current) {
      drawerFourRef.current.position.lerp(
        targetPositionFour.current,
        delta * 5
      );
    }
  ...
  const Cylinder = ({ ...props }): JSX.Element => {
    return (
      ...
      >
        <cylinderGeometry
          args={[props.radiusTop, props.radiusBottom, props.height]}
        />
        <meshToonMaterial
          color={props.color}
          opacity={props.opacity}
        ...
        <group position={[-0.34, 0.23, 0]} rotation={[0, 0, (2 * Math.PI) / 3]}>
          ...
          {us_laptopOn && (
            <mesh
              ...
              onClick={() =>
                window.open(
                  "https://www.linkedin.com/in/brady-r-richardson/",
                  "_blank"
                )
              }
            >
          ...
          {us_laptopOn && (
            <mesh
              ...
              rotation={[Math.PI / 2, 0, (3 * Math.PI) / 2]}
              onClick={() =>
                window.open("https://github.com/bradyrichardson", "_blank")
              }
            >
              <planeGeometry args={[0.5, 0.5]} />
              <meshBasicMaterial
    ...
    ({ leftRef, rightRef, rotations, ...props }, ref): JSX.Element => {
      useFrame((_, delta) => {
        if (leftRef.current && rightRef.current) {
          ...
          rotations.current.left +=
            (rotations.current.target - rotations.current.left) * delta * 5;
          rotations.current.right +=
            (rotations.current.target - rotations.current.right) * delta * 5;
      ...
      const handleWindowMouseOver = () => {
        ...
        rotations.current.target =
          Math.abs(rotations.current.target) < 0.01 ? Math.PI / 3 : 0;
      };
      ...
      return (
        ...
        <group position={props.position} rotation={props.rotation}>
          ...
          <mesh>
            ...
            {/* Window Sill  */}
            {Box({
              position: [0.012, 1.8, -2.5],
              geometry: [1.6, 0.1, 0.5],
              onPointerOver: handleWindowMouseOver,
              emissive: COLOR_PALETTE.beige,
              emissiveIntensity: !us_lightOne && !us_stringLightsOn ? 0.5 : 0,
            })}
          </mesh>
  ...
  const StringLights = ({ ...props }) => {
    return (
      <group {...props}>
        ...
        {/* Lights */}
        <StringLight
          position={[1.62, 3.8, -2.0]}
          color={COLOR_PALETTE.lightYellow}
        />
        <StringLight
          position={[1.62, 3.7, -1.5]}
          color={COLOR_PALETTE.lightYellow}
        />
        {!props.isEnd ? (
          <StringLight
            position={[1.62, 3.8, -1.0]}
            color={COLOR_PALETTE.lightYellow}
          />
        ) : (
          <></>
  ...
  const ProjectDisplayBoard = forwardRef(
    (
      {
        ...props
      }: {
      ...
      useFrame(() => {
        ...
        if (
          projectDisplayBoardOneRef.current &&
          projectDisplayBoardOneIsAnimating.current &&
          targetPositionOne.current.x >= 0.59
        ) {
          projectDisplayBoardOneRef.current.position.lerp(
            displayBoardOneEndPosition,
            lerpOpenDelta
          );
        } else if (
          projectDisplayBoardOneRef.current &&
        ...
        if (
          projectDisplayBoardTwoRef.current &&
          projectDisplayBoardTwoIsAnimating.current &&
          targetPositionTwo.current.x >= 0.59
        ) {
          projectDisplayBoardTwoRef.current.position.lerp(
            displayBoardTwoEndPosition,
            lerpOpenDelta
          );
        } else if (
          projectDisplayBoardTwoRef.current &&
        ...
        if (
          projectDisplayBoardThreeRef.current &&
          projectDisplayBoardThreeIsAnimating.current &&
          targetPositionThree.current.x >= 0.59
        ) {
          projectDisplayBoardThreeRef.current.position.lerp(
            displayBoardThreeEndPosition,
            lerpOpenDelta
          );
        } else if (
          projectDisplayBoardThreeRef.current &&
        ...
        if (
          projectDisplayBoardFourRef.current &&
          projectDisplayBoardFourIsAnimating.current &&
          targetPositionFour.current.x >= 0.59
        ) {
          projectDisplayBoardFourRef.current.position.lerp(
            displayBoardFourEndPosition,
            lerpOpenDelta
          );
        } else if (
          projectDisplayBoardFourRef.current &&
  ...
  const FlatBook = ({ ...props }) => {
    ...
    useFrame((_, delta) => {
      if (leftCoverRef.current && rightCoverRef.current) {
        ...
        const newRotation =
          currentRotation +
          (targetRotation.current - currentRotation) * delta * 5;
        ...
        <group ref={rightCoverRef} position={[0.25, 0.03, 0]}>
          <mesh position={[-0.25, 0, 0]} rotation={[-Math.PI / 2, 0, 0]}>
            ...
            <meshStandardMaterial
              color={COLOR_PALETTE.silver}
              emissive={COLOR_PALETTE.beige}
              emissiveIntensity={!us_stringLightsOn && !us_lightOne ? 0.5 : 0}
            />
          </mesh>
        </group>
  ...
  return (
    <group ref={group} {...props} dispose={null}>
      ...
      {Box({
        position: [0, -0.02, 0],
        geometry: [5.1, 0.1, 5.1],
        color: COLOR_PALETTE.test,
      })}
      {/* Walls */}
      {/* Right Wall */}
      {Box({
        position: [-1.62, 0.9, -2.5],
        geometry: [1.85, 1.75, 0.1],
        color: COLOR_PALETTE.darkBlue,
      })}
      {Box({
        position: [-1.65, 2.45, -2.5],
        geometry: [1.8, 1.75, 0.1],
        color: COLOR_PALETTE.darkBlue,
      })}
      {Box({
        position: [-1.62, 4.075, -2.5],
        geometry: [1.85, 1.75, 0.1],
        color: COLOR_PALETTE.darkBlue,
      })}
      {Box({
        position: [1.675, 2.45, -2.5],
        geometry: [1.75, 1.75, 0.1],
        color: COLOR_PALETTE.darkBlue,
      })}
      {Box({
        position: [1.675, 4.075, -2.5],
        geometry: [1.75, 1.75, 0.1],
        color: COLOR_PALETTE.darkBlue,
      })}
      {Box({
        position: [1.675, 0.9, -2.5],
        geometry: [1.75, 1.75, 0.1],
        color: COLOR_PALETTE.darkBlue,
      })}
      {Box({
      ...
      {Box({
        position: [0, 4.075, -2.5],
        geometry: [1.75, 1.75, 0.1],
        color: COLOR_PALETTE.darkBlue,
      })}
      {/* Left Wall */}
      {Box({
        position: [-2.5, 0.9, -1.62],
        geometry: [0.1, 1.75, 1.75],
        color: COLOR_PALETTE.darkBlue,
      })}
      {Box({
        position: [-2.5, 2.45, -1.62],
        geometry: [0.1, 1.75, 1.75],
        color: COLOR_PALETTE.darkBlue,
      })}
      {Box({
        position: [-2.5, 4.075, -1.62],
        geometry: [0.1, 1.75, 1.75],
        color: COLOR_PALETTE.darkBlue,
      })}
      {Box({
        position: [-2.5, 2.45, 1.675],
        geometry: [0.1, 1.75, 1.75],
        color: COLOR_PALETTE.darkBlue,
      })}
      {Box({
        position: [-2.5, 4.075, 1.675],
        geometry: [0.1, 1.75, 1.75],
        color: COLOR_PALETTE.darkBlue,
      })}
      {Box({
        position: [-2.5, 0.9, 1.675],
        geometry: [0.1, 1.75, 1.75],
        color: COLOR_PALETTE.darkBlue,
      })}
      {Box({
      ...
      {Box({
        position: [-2.4, 0.5, -0.95],
        geometry: [0.1, 1, 0.1],
        color: COLOR_PALETTE.beige,
      })}
      {Box({
        position: [-1.55, 0.5, -2.4],
        geometry: [0.1, 1, 0.1],
        color: COLOR_PALETTE.beige,
      })}
      {Box({
        position: [-1.55, 0.5, -0.95],
        geometry: [0.1, 1, 0.1],
        color: COLOR_PALETTE.beige,
      })}
      {/* Chair */}
      ...
      {/* Bed Frame */}
      {Box({
        position: [1.7, 0.25, -2.35],
        geometry: [1.55, 0.1, 0.05],
        color: COLOR_PALETTE.beige,
      })}
      {Box({
        position: [1.7, 0.25, -2.05],
        geometry: [1.55, 0.1, 0.05],
        color: COLOR_PALETTE.beige,
      })}
      {Box({
        position: [1.7, 0.25, -1.75],
        geometry: [1.55, 0.1, 0.05],
        color: COLOR_PALETTE.beige,
      })}
      {Box({
        position: [1.7, 0.25, -1.45],
        geometry: [1.55, 0.1, 0.05],
        color: COLOR_PALETTE.beige,
      })}
      {Box({
        position: [1.7, 0.25, -1.15],
        geometry: [1.55, 0.1, 0.05],
        color: COLOR_PALETTE.beige,
      })}
      {Box({
        position: [1.7, 0.25, -0.85],
        geometry: [1.55, 0.1, 0.05],
        color: COLOR_PALETTE.beige,
      })}
      {Box({
        position: [1.7, 0.25, -0.55],
        geometry: [1.55, 0.1, 0.05],
        color: COLOR_PALETTE.beige,
      })}
      {Box({
        position: [1.7, 0.25, -0.25],
        geometry: [1.55, 0.1, 0.05],
        color: COLOR_PALETTE.beige,
      })}
      {Box({
        position: [1.7, 0.25, 0.05],
        geometry: [1.55, 0.1, 0.05],
        color: COLOR_PALETTE.beige,
      })}
      {Box({
        position: [1.7, 0.25, 0.35],
        geometry: [1.55, 0.1, 0.05],
        color: COLOR_PALETTE.beige,
      })}
      {Box({
        position: [1.7, 0.25, 0.65],
        geometry: [1.55, 0.1, 0.05],
        color: COLOR_PALETTE.beige,
      })}
      {/* Bed Frame Border */}
      {Box({
        position: [2.5, 0.25, -0.85],
        geometry: [0.1, 0.1, 3.2],
        color: COLOR_PALETTE.beige,
      })}
      {Box({
        position: [0.89, 0.25, -0.85],
        geometry: [0.1, 0.1, 3.2],
        color: COLOR_PALETTE.beige,
      })}
      {Box({
        position: [1.696, 0.25, 0.72],
        geometry: [1.71, 0.1, 0.1],
        color: COLOR_PALETTE.beige,
      })}
      {Box({
        position: [1.696, 0.25, -2.4],
        geometry: [1.71, 0.1, 0.1],
        color: COLOR_PALETTE.beige,
      })}
      {/* Bed Legs */}
      {Box({
        position: [0.89, 0.1, -2.4],
        geometry: [0.1, 0.25, 0.1],
        color: COLOR_PALETTE.beige,
      })}
      {Box({
        position: [2.499, 0.1, -2.4],
        geometry: [0.1, 0.25, 0.1],
        color: COLOR_PALETTE.beige,
      })}
      {Box({
        position: [0.89, 0.1, 0.72],
        geometry: [0.1, 0.25, 0.1],
        color: COLOR_PALETTE.beige,
      })}
      {Box({
        position: [2.499, 0.1, 0.72],
        geometry: [0.1, 0.25, 0.1],
        color: COLOR_PALETTE.beige,
      })}
      {/* Pillow */}
      ...
      {Box({
        position: [-1.75, 1, 2.499],
        geometry: [1.5, 2, 0.1],
        color: COLOR_PALETTE.darkBrown,
      })}
      {Box({
        position: [-1.75, 1.95, 1.799],
        geometry: [1.5, 0.1, 1.5],
        color: COLOR_PALETTE.darkBrown,
      })}
      {Box({
        position: [-1.75, 1.95, 1.799],
      ...
      {Box({
        position: [-1.75, 0.15, 1.799],
        geometry: [0.5, 0.2, 1.5],
        color: COLOR_PALETTE.darkBrown,
      })}
      {/* Drawers */}
      ...
      {Box({
        position: [-2.33, 3.68, 0],
        geometry: [0.4, 0.06, 3],
        color: COLOR_PALETTE.beige,
      })}
      {Box({
        position: [-2.33, 4, 1.475],
        geometry: [0.4, 0.6, 0.05],
        color: COLOR_PALETTE.beige,
      })}
      {Box({
        position: [-2.33, 4, -1.475],
      ...
      {/* Lacrosse Stick */}
      {LacrosseStick({
        position: [-2, 0.54, -0.55],
        rotation: [2.6, -2 * Math.PI, -Math.PI],
      })}
      {/* Lacrosse Ball */}
      <mesh
        ref={ballRef}
      ...
      >
        ...
        <meshToonMaterial
          ...
          emissive={COLOR_PALETTE.beige}
          emissiveIntensity={
            !us_lightOne && !us_lightTwo && !us_laptopOn ? 0.5 : 0
          }
        />
      </mesh>
      {/* Windows */}
      ...
      {StringLights({
        position: [-0.35, 1, -0.73],
        rotation: [0, Math.PI / 2, 0],
      })}
      {StringLights({
        position: [0.65, 1, -0.73],
        rotation: [0, Math.PI / 2, 0],
      })}
      {StringLights({
        position: [1.65, 1, -0.73],
        rotation: [0, Math.PI / 2, 0],
      })}
      {StringLights({
        position: [2.65, 1, -0.73],
        rotation: [0, Math.PI / 2, 0],
      })}
      {StringLights({
        position: [3.65, 1, -0.73],
        rotation: [0, Math.PI / 2, 0],
        isEnd: true,
      })}
      ...
      {/* Help text */}
      {us_projectsFont && (
        <group dispose={null} position={[3, 3, 0]}>
          <mesh
            scale={[1, 1, 0.001]}
            {...props}
            position={[0, 0, 0]}
      ...
      {us_projectsFont && (
        <group dispose={null} position={[3, 2, 0]}>
          <mesh
            scale={[1, 1, 0.001]}
            {...props}
            position={[0, 0, 0]}
      ...
      {us_projectsFont && (
        <group dispose={null} position={[3, 1, 0]}>
          <mesh
            scale={[1, 1, 0.001]}
            {...props}
            position={[0, 0, 0]}
